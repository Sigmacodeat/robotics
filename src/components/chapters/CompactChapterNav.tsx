...
export default function CompactChapterNav({ currentChapter, orientation = "horizontal" }: { currentChapter?: number; orientation?: Orientation }) {
  ...
  const linkBase = clsx(
    "relative inline-flex items-center rounded-xl transition-all duration-300 focus:outline-none focus-visible:outline-none hover:shadow-sm",
                ...
                <div className="relative">
                  ...
                  >
                    ...
                    {!isHorizontal && c.subchapters?.length ? (
                      <button
                        type="button"
                        aria-label={openChapter === c.id ? (locale?.startsWith('de') ? 'Unterkapitel einklappen' : 'Collapse subchapters') : (locale?.startsWith('de') ? 'Unterkapitel ausklappen' : 'Expand subchapters')}
                        ...
                        className="ml-2 inline-flex h-6 w-6 shrink-0 items-center justify-center rounded-md text-[--color-foreground-muted] hover:text-[--color-foreground] ring-1 ring-transparent hover:ring-[--color-border-subtle]"
                      >
                        <svg
