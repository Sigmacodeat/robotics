...
export default function TechWorkPackages({ title, packages }: TechWorkPackagesProps) {
  ...
  return (
    <div className="mt-6">
      {title ? (
        <>
          <h3 className="font-semibold m-0 pb-1" style={{ backgroundImage: 'none', textDecoration: 'none' }}>{title}</h3>
          <div className="mt-1 h-px w-full" style={{ backgroundColor: 'var(--underline-neutral-end)' }} aria-hidden />
        </>
      ) : null}
      <TableSimple
        headers={[
