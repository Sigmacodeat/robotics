...
import ExecutiveChapter from "./chapter/ExecutiveChapter";
import MarketChapter from "./chapter/MarketChapter";
import ProblemChapter from "./chapter/ProblemChapter";
import SolutionChapter from "./chapter/SolutionChapter";
import TechnologyChapter from "./chapter/TechnologyChapter";
import ProductsChapter from "./chapter/ProductsChapter";
import BusinessModelChapter from "./chapter/BusinessModelChapter";
import FinanceChapter from "./chapter/FinanceChapter";
import RisksChapter from "./chapter/RisksChapter";
import ComplianceChapter from "./chapter/ComplianceChapter";
// StateOfTheArtChapter consolidated into TechnologyChapter
import CompanyChapter from "./chapter/CompanyChapter";
import TeamOrgChapter from "./chapter/TeamOrgChapter";
import GTMChapter from "./chapter/GTMChapter";
import { formatInteger } from "@/lib/format";
// RiskExitChapter consolidated into RisksChapter
// MarketCompetitiveChapter consolidated into MarketChapter
// InnovationChapter consolidated into TechnologyChapter
// IPMoatChapter consolidated into TechnologyChapter
import CostPlanChapter from "./chapter/CostPlanChapter";
import CapTableChapter from "./chapter/CapTableChapter";
import DisseminationChapter from "./chapter/DisseminationChapter";
// ResponsibleAIChapter consolidated into ComplianceChapter
// MLOpsChapter consolidated into Operations (combined under MLOps & Operations chapter)
// OperationsChapter consolidated into MLOps & Operations
import CTAChapter from "./chapter/CTAChapter";
import ImpactChapter from "./chapter/ImpactChapter";
import AppendixChapter from "./chapter/AppendixChapter";
import Chapter from "./chapter/Chapter";
import MarketCompetitiveChapter from "./chapter/MarketCompetitiveChapter";
import Link from "next/link";
import TractionKPIsChapter from "./chapter/TractionKPIsChapter";
import { z } from "zod";
import deBp from "@/i18n/locales/de/bp";
import enBp from "@/i18n/locales/en/bp";
...
export default function BusinessPlanSections() {
  ...
  const messages = (locale.startsWith("de") ? (deBp as unknown) : (enBp as unknown)) as Record<string, unknown>;
    ...
    <div className="">
        ...
        parentSectionNumber={2}
        {
          ...(() => {
            try {
              ...
              const sol = (exec?.sections as any)?.solution ?? {};
      ...
      {(() => {
        try {
          ...
          let revenueStreamsDetailed = (bm as any)?.revenueStreamsDetailed as { type: string; description: string }[] | undefined;
          ...
          if (!Array.isArray(revenueStreamsDetailed) || revenueStreamsDetailed.length === 0) {
            if (Array.isArray(revenueStreamsRaw) && revenueStreamsRaw.every((x: any) => x && typeof x === 'object' && 'type' in x && 'description' in x)) {
              revenueStreamsDetailed = revenueStreamsRaw as { type: string; description: string }[];
            }
          }
          const pricingModel = (bm as any)?.pricingModel as { model: string; description: string }[] | undefined;
          const costStructure = (bm as any)?.costStructure as { category: string; items: string[] }[] | undefined;
          const keyMetrics = (bm as any)?.keyMetrics as { metric: string; target: string }[] | undefined;
          const competitiveAdvantage = (bm as any)?.competitiveAdvantage as string[] | undefined;
          const gtmPoints = (bm as any)?.gtmPoints as string[] | undefined;
          const grantFitPoints = (bm as any)?.grantFitPoints as string[] | undefined;
          const moatPoints = (bm as any)?.moatPoints as string[] | undefined;
